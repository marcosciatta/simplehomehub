import {DeviceAddedEvent, DeviceUpdatedEvent, DeviceChangedStateEvent, DeviceChangedAttributesEvent} from './events';
import {deviceStateTraker} from './utils/deviceProxy';
import ObservableSlim  from 'observable-slim';
import BaseDevice from '../devices/baseDevice.mjs';
...
class Home {
    constructor({logger, messagebus, componentRegistry}) {
        ...
        this.messagebus.subscribe({
            ...
            callback: (data, envelope) => {
                this.changeDeviceStateTo(data.identity, data.operation);
            }
        });
    }
    ...
    changeDeviceAttributes(identity, operations) {
        this.logger.debug('Change state to device [' + identity + '] to %j', operations);
        if (identity) {
            let device = this.getDevice(identity);

            if(!device.supportAttributes(operations)){
                return;
            }

            let evt = new DeviceChangedAttributesEvent(
                device.identity,
                device.realm,
                operations
            );
            ...
            // if(device.getComType() == BaseDevice.COM_TYPE_ASYNC){
            //     //do something async
            // } else  {
                this.messagebus.publish(evt);
                device.setAttributes(operations);
            // }
        }
    }
